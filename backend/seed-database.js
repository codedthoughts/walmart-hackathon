...
const productsData = [
    { product_id: "PROD001", name: "Fresh Milk 1L", category: "Dairy", selling_price: 50.0, cost_price: 30.0, is_perishable: true, shelf_life_days: 7, base_daily_sales: 30 },
    { product_id: "PROD002", name: "Whole Wheat Bread", category: "Bakery", selling_price: 40.0, cost_price: 25.0, is_perishable: true, shelf_life_days: 4, base_daily_sales: 25 },
    { product_id: "PROD003", name: "Cheddar Cheese 200g", category: "Dairy", selling_price: 150.0, cost_price: 100.0, is_perishable: true, shelf_life_days: 30, base_daily_sales: 10 },
    { product_id: "PROD004", name: "Cola 2L", category: "Beverages", selling_price: 90.0, cost_price: 60.0, is_perishable: false, shelf_life_days: 365, base_daily_sales: 40 },
    { product_id: "PROD005", name: "Lays Chips Classic", category: "Snacks", selling_price: 20.0, cost_price: 12.0, is_perishable: false, shelf_life_days: 180, base_daily_sales: 50 },
    { product_id: "PROD006", name: "Fresh Apples 1kg", category: "Produce", selling_price: 120.0, cost_price: 80.0, is_perishable: true, shelf_life_days: 10, base_daily_sales: 15 },
    { product_id: "PROD007", name: "Detergent 1kg", category: "Household", selling_price: 250.0, cost_price: 180.0, is_perishable: false, shelf_life_days: 730, base_daily_sales: 8 },
];
...
const seedDB = async () => {
    try {
        ...
        await Promise.all([
            Product.deleteMany({}), Sale.deleteMany({}), Weather.deleteMany({}),
            Inventory.deleteMany({}), Alert.deleteMany({}), Forecast.deleteMany({})
        ]);
        ...
        for (let i = 180; i >= 1; i--) {
            ...
            // --- THIS IS THE CORRECTED SALES GENERATION LOGIC THAT WAS MISSING ---
            for (const p of productsData) {
                let units_sold = p.base_daily_sales;
                if (dayOfWeek === 0 || dayOfWeek === 6) { // Weekends
                    units_sold *= (p.category === 'Snacks' || p.category === 'Beverages') ? 1.8 : 1.3;
                }
                if (temperature_c > 30 && p.category === 'Beverages') {
                    units_sold *= 1.5;
                }
                ...
                salesToInsert.push({
                    ...
                    date,
                    units_sold: Math.max(0, Math.floor(units_sold)),
        ...
        // ... (The rest of the logic for engineering the demo scenarios is correct and remains unchanged)
        ...
        for (const p of productsData) {
            ...
            inventoryToInsert.push({ product_id: p.product_id, quantity: initialQuantity, current_price: p.selling_price, batch_id: `BATCH-${p.product_id}-INITIAL`, received_date: new Date(), expiry_date });
        }
